"use strict";
var jsxRuntime = require("react/jsx-runtime"), require$$0 = require("react"), hooks = require("./hooks.cjs"), comlink = require("@sanity/comlink"), presentationComlink = require("@sanity/presentation-comlink");
...
function useLiveEvents(client) {
  const [state, dispatch] = require$$0.useReducer(reducer$1, initialState), [error, setError] = require$$0.useState(null);
  if (error !== null)
    throw error;
  ...
  }, [client.live]), require$$0.useDeferredValue(state);
}
...
function useLiveQueries() {
  const [queries, dispatch] = require$$0.useReducer(reducer, initialQueries), [snapshots] = require$$0.useState(() => /* @__PURE__ */ new Map()), subscribe2 = require$$0.useCallback((payload) => (dispatch({ type: "subscribe", payload }), () => dispatch({ type: "unsubscribe", payload })), []), update = require$$0.useCallback(
...
function usePerspective(initialPerspective) {
  const [presentationPerspective, setPresentationPerspective] = require$$0.useState(null);
  return require$$0.useEffect(() => {
...
function LiveStoreProvider(props) {
  ...
  const perspective = usePerspective(props.perspective || "drafts"), [client] = require$$0.useState(() => {
  ...
  }), [logger] = require$$0.useState(() => props.logger);
  require$$0.useEffect(() => {
    logger && logger.log(
      "[@sanity/preview-kit]: Updates will be applied in real-time using the Sanity Live Content API."
    );
  }, [logger]);
  const { queries, snapshots, subscribe: subscribe2, update } = useLiveQueries(), context = require$$0.useMemo(() => function(initialSnapshot, query, params) {
...
function QuerySubscription(props) {
  const {
    ...
    syncTags,
    listeners
  } = props, [skipEventIds] = require$$0.useState(() => new Set(liveEventsMessages.map((msg) => msg.id))), recentLiveEvents = require$$0.useMemo(
    () => liveEventsMessages.filter((msg) => !skipEventIds.has(msg.id)),
    [liveEventsMessages, skipEventIds]
  ), lastLiveEventId = require$$0.useMemo(
    () => recentLiveEvents.findLast((msg) => msg.tags.some((tag) => syncTags?.includes(tag))),
    [recentLiveEvents, syncTags]
  )?.id, [error, setError] = require$$0.useState(null);
