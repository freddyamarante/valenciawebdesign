...
function getEventSelection({
  ...
  return range ? editorProvider.toPortableTextRange(editorProvider.fromSlateValue(slateEditor.children, schema.block.name), range, schema) : null;
}
...
const debug = editorProvider.debugWithName("component:Editable"), PLACEHOLDER_STYLE = {
  ...
  const renderElement = React.useCallback((eProps) => /* @__PURE__ */ jsxRuntime.jsx(Element$1, { ...eProps, readOnly, renderBlock, renderChild, renderListItem, renderStyle, schemaTypes, spellCheck }), [schemaTypes, spellCheck, readOnly, renderBlock, renderChild, renderListItem, renderStyle]), renderLeaf = React.useCallback((lProps) => {
